...
from model_05.network_05 import Net
import torch
...
model_cp = './model/model05/'               # 网络参数保存位置
...
def train():
    trainLog = open('trainLog05.log', mode='a', encoding='utf-8')
    ...
    print('Dataset loaded! length of train set is {0}'.format(len(datafile)), file=trainLog)
    ...
    for epoch in range(nepoch):
        ...
        for img, label in dataloader:                                           # 循环读取封装后的数据集，其实就是调用了数据集中的__getitem__()方法，只是返回数据格式进行了一次封装
            ...
            print('Epoch,{0},Frame,{1},train_loss,{2}'.format(epoch, cnt*batch_size, loss/batch_size), file=trainLog)  # 打印一个batch size的训练结果
